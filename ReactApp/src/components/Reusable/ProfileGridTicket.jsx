...
export default class ProfileGridTicket extends Component {
  ...
  componentDidMount() {
    instance
      .get(
        "http://localhost:8080/db/tickets?username=" +
  ...
  compareDifficulty = (a, b) => {
    if (a.difficulty == "Senior") {
      return -1;
    } else if (b.difficulty == "Senior") {
      return 1;
    } else if (a.difficulty == "Intern") {
      return 1;
    } else if (b.difficulty == "Intern") {
      return -1;
    } else {
      return 0;
    }
    ...
    this.state.tickets.map((x) =>
      tickets.push(
        <Grid item xs={6} sm={4} md={3}>
          <Ticket for="user" ticket={x} grid={this.update}></Ticket>{" "}
        </Grid>
      )
    );
    return tickets;
